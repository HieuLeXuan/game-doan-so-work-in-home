...
<div class="container mt-5">
  <div class="row">
    ...
    <table class="table">
      ...
      <tbody>
      <c:forEach items="${games}" var="game" varStatus="i">
        <tr>
          ...
          <td>${game.endTime}</td>
          <td>${game.isCompleted}</td>
          <td>${game.isActive}</td>
          <form method="get"
                action="<%=request.getContextPath() + UrlUtils.GAME%>">
            <c:choose>
              <c:when test="${game.isCompleted != 'true'}">
                <td>
                  <input type="hidden" name="typeFunction" value="continue">
                  <input type="hidden" name="gameSessionId"
                         value="${game.id}">
